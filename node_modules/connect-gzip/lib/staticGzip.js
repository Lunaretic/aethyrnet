...
exports = module.exports = function staticGzip(root, options) {
  ...
  //If we have a gzPath, we need to scan the root,
  //and the gzPath to validate the directory structure
  if(options.gzPath)
  {
    var gzPath = options.gzPath.split('/');
    for(var idx in gzPath)
    {
      //var cPath = 
      //fs.existsSync(
    }
  ...
  return function(req, res, next) {
    ...
    fs.stat(filename, function(err, stat) {
      ...
      var gzipname = path.join(dir, base + '.' + Number(stat.mtime) + '.gz');
      
      //If we have a GZip Path
      if(options.gzPath)
      {
        //Create a proper path to the Gzip folder structure 
        var fullGzPath = path.dirname(path.join(options.gzPath, url.pathname));
        gzipname = path.join(fullGzPath,  base + '.' + Number(stat.mtime) + '.gz');
      }
      ...
      function sendGzip() {
        ...
        passToStatic(gzipname, true);
      }
    });
    
    function passToStatic(name, gZip) {
      var o = Object.create(options);
      if(gZip && options.gzPath)
      {
        o.path = path.basename(name);
        o.root = path.dirname(name);
      }
      else
